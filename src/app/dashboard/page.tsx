...
interface CashBreakdownItem {
    ...
    buyerName?: string; quantity?: number;
}
...
export default function DashboardPage() {
    ...
    const fetchData = async (searchFilters = filters) => {
        ...
        try {
            ...
            const [sinceRes, yRes, monthStatsRes] = await Promise.all([
                fetch(`/api/reports?startDate=${sinceDate}&endDate=${now}`),
                fetch(`/api/reports?startDate=${yStart.toISOString()}&endDate=${yEnd.toISOString()}`),
                fetch(`/api/reports?startDate=${monthStart.toISOString()}&endDate=${now}`),
            ]);
            if (sinceRes.ok) { const d = await sinceRes.json(); setSinceStats(d.range); }
            if (yRes.ok) setYesterdayStats(await yRes.json());
            if (monthStatsRes.ok) { const d = await monthStatsRes.json(); setMonthModelBreakdown(d.range?.modelBreakdown ?? null); }
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDeposit = async () => {
        ...
        const cashInHand = Math.max(0, (sinceStats.cashReceived ?? 0) + (sinceStats.khataCashReceived ?? 0) + (sinceStats.usedBikeCashReceived ?? 0) - (sinceStats.expenseCash ?? 0) + extra);
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '1.25rem' }}>
                            ...
                            {Object.entries(stats.allTime.modelBreakdown)
                                ...
                                .map(([model, allTimeCount]) => {
                                    const monthCount = monthModelBreakdown?.[model] ?? 0;
                                    ...
                                    return (
                                        <div key={model} className="card" style={{ padding: '0.85rem 1.25rem', display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: '130px', flex: '1 1 130px', maxWidth: '200px' }}>
                                            ...
                                            <div>
                                                ...
                                                <div style={{ fontSize: '1.6rem', fontWeight: 800, color: 'var(--color-primary)', lineHeight: 1.1 }}>{monthCount}</div>
                                                <div style={{ fontSize: '0.65rem', color: 'var(--color-text-muted)' }}>
                                                    this month{todayCount > 0 ? ` · +${todayCount} today` : ''}
                                                </div>
                                                <div style={{ fontSize: '0.6rem', color: 'var(--color-text-muted)', marginTop: '0.15rem' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        <div className="card" style={{ padding: '1.25rem', flex: 1, borderLeft: '4px solid #f59e0b' }}>
                            ...
                            <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#f59e0b', marginBottom: '0.75rem' }}>
                                Rs. {Math.max(0, (sinceStats?.cashReceived ?? 0) + (sinceStats?.khataCashReceived ?? 0) + (sinceStats?.usedBikeCashReceived ?? 0) - (sinceStats?.expenseCash ?? 0) + (Number(manualCash) || 0)).toLocaleString()}
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem', fontSize: '0.82rem', marginBottom: '0.75rem' }}>
                                ...
                                {(sinceStats?.usedBikeCashReceived ?? 0) > 0 && (
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        <span style={{ color: 'var(--color-text-muted)' }}>Used Bike Resale</span>
                                        <strong style={{ color: '#10b981' }}>+ Rs. {(sinceStats?.usedBikeCashReceived ?? 0).toLocaleString()}</strong>
                                    </div>
                                )}
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                {(sinceStats?.expenseCash ?? 0) > 0 && (
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        <span style={{ color: '#ef4444' }}>Cash Expenses</span>
                                        <strong style={{ color: '#ef4444' }}>− Rs. {(sinceStats?.expenseCash ?? 0).toLocaleString()}</strong>
                                    </div>
                                )}
                            ...
                            <button
                                ...
                                disabled={depositing || Math.max(0, (sinceStats?.cashReceived ?? 0) + (sinceStats?.khataCashReceived ?? 0) + (sinceStats?.usedBikeCashReceived ?? 0) - (sinceStats?.expenseCash ?? 0) + (Number(manualCash) || 0)) <= 0}
                                className="btn"
                                style={{ width: '100%', fontSize: '0.8rem', padding: '0.4rem', background: '#f59e0b', color: '#fff', border: 'none', borderRadius: '6px', fontWeight: 700, cursor: depositing ? 'not-allowed' : 'pointer', opacity: Math.max(0, (sinceStats?.cashReceived ?? 0) + (sinceStats?.khataCashReceived ?? 0) + (sinceStats?.usedBikeCashReceived ?? 0) - (sinceStats?.expenseCash ?? 0)) <= 0 ? 0.5 : 1 }}>
                                ...
                                <table style={{ width: 'max-content', minWidth: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
                                    <thead>
                                        <tr style={{ background: 'var(--color-bg-elevated)' }}>
                                            {['#', 'Buyer', 'Model', 'Mode', 'Sale Price', 'Std Price', 'Rcvd Cash', 'Bank Xfer', 'Base Margin', 'Extra', 'Bike Profit', 'Reg Profit', 'Total Profit'].map(h => (
                                                <th key={h} style={{ padding: '0.4rem 0.6rem', textAlign: h === 'Buyer' ? 'left' : 'right', fontWeight: 700, color: 'var(--color-text-muted)', whiteSpace: 'nowrap', borderBottom: '1px solid var(--color-border)' }}>{h}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                    ...
                                    <tfoot>
                                        <tr style={{ borderTop: '2px solid var(--color-border)', fontWeight: 700, background: 'var(--color-bg-elevated)' }}>
                                            <td colSpan={6} style={{ padding: '0.5rem 0.6rem', textAlign: 'right', fontSize: '0.8rem' }}>TOTALS</td>
                        ...
                        {show && (
                            <div style={{ overflowX: 'auto' }}>
                                <table style={{ width: 'max-content', minWidth: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
                                    <thead>
                                        <tr style={{ background: 'var(--color-bg-elevated)' }}>
                                            {['#', 'Party / Buyer', 'Model', 'Qty', 'Mode', 'Sale Price', 'Std Price', 'Rcvd Cash', 'Bank Xfer', 'Base Margin', 'Extra', 'Bike Profit', 'Reg Profit', 'Total Profit'].map(h => (
                                                <th key={h} style={{ padding: '0.4rem 0.6rem', textAlign: h === 'Party / Buyer' ? 'left' : 'right', fontWeight: 700, color: 'var(--color-text-muted)', whiteSpace: 'nowrap', borderBottom: '1px solid var(--color-border)' }}>{h}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {data!.map((row, i) => {
                                            ...
                                            return (
                                                <tr key={i} style={{ borderBottom: '1px solid var(--color-border)', background: row.paymentMode === 'KHATA' ? 'rgba(139,92,246,0.04)' : i % 2 === 0 ? 'transparent' : 'var(--color-bg-elevated)' }}>
                                                    ...
                                                    <td style={{ padding: '0.4rem 0.6rem', textAlign: 'right' }}>
                                                        <span style={{ fontSize: '0.65rem', padding: '1px 5px', borderRadius: '4px',
                                                            background: row.paymentMode === 'KHATA' ? 'rgba(139,92,246,0.15)' : row.paymentMode === 'ADVANCE' ? 'rgba(245,158,11,0.15)' : row.paymentMode === 'CREDIT' ? 'rgba(239,68,68,0.12)' : row.paymentMode === 'BANK_TRANSFER' ? 'rgba(59,130,246,0.12)' : 'rgba(16,185,129,0.12)',
                                                            color: row.paymentMode === 'KHATA' ? '#8b5cf6' : row.paymentMode === 'ADVANCE' ? '#f59e0b' : row.paymentMode === 'CREDIT' ? '#ef4444' : row.paymentMode === 'BANK_TRANSFER' ? '#3b82f6' : '#10b981' }}>
                                                            {row.paymentMode}
                                                        </span>
                                                    </td>
                                                    <td style={{ padding: '0.4rem 0.6rem', textAlign: 'right' }}>{row.price.toLocaleString()}</td>
                                                    <td style={{ padding: '0.4rem 0.6rem', textAlign: 'right', color: 'var(--color-text-muted)' }}>{row.standardPrice.toLocaleString()}</td>
                                                    <td style={{ padding: '0.4rem 0.6rem', textAlign: 'right', color: '#10b981' }}>{row.receivedCash > 0 ? row.receivedCash.toLocaleString() : '-'}</td>
                                                    <td style={{ padding: '0.4rem 0.6rem', textAlign: 'right', color: '#3b82f6' }}>{row.bankTransferAmount > 0 ? row.bankTransferAmount.toLocaleString() : '-'}</td>
                                                    <td style={{ padding: '0.4rem 0.6rem', textAlign: 'right', color: 'var(--color-text-muted)' }}>{row.baseMargin.toLocaleString()}</td>
                                                    <td style={{ padding: '0.4rem 0.6rem', textAlign: 'right', color: extra > 0 ? '#10b981' : extra < 0 ? '#ef4444' : 'var(--color-text-muted)' }}>
                                    ...
                                    <tfoot>
                                        <tr style={{ borderTop: '2px solid var(--color-border)', fontWeight: 700, background: 'var(--color-bg-elevated)' }}>
                                            <td colSpan={7} style={{ padding: '0.5rem 0.6rem', textAlign: 'right', fontSize: '0.8rem' }}>TOTALS</td>
